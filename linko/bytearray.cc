...
bool ByteArray::isLittleEndian() const {
    return m_endian == LINKO_LITTLE_ENDIAN;
}

void ByteArray::setIsLittleEndian(bool val) {
    if (val) {
        m_endian = LINKO_LITTLE_ENDIAN;
    } else {
        m_endian = LINKO_BIG_ENDIAN;
    }
}
...
bool ByteArray::writeToFile(const std::string& name) const {
    ...
    if (!ofs) {
        LINKO_LOG_ERROR(g_logger) << "writeToFile name=" << name
    ...
    while (read_size > 0) {
        ...
        if (len + diff == (int64_t)m_baseSize) {
            cur = cur->next;
        }
        pos += len;
        read_size -= len;
...
bool ByteArray::readFromFile(const std::string& name) {
    ...
    if (!ifs) {
        LINKO_LOG_ERROR(g_logger) << "readFromFile name=" << name
            << " error, errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
...
void ByteArray::addCapacity(size_t size) {
    ...
    size = size - old_cap;
    size_t count = (size / m_baseSize) + ((size % m_baseSize) > 0 ? 1 : 0);
    Node* tmp = m_root;
    while (tmp->next) {
        tmp = tmp->next;
    }
